...
sealed interface Type {
    ...
    companion object {
        val identityEntries = listOf(AV, CV, Live, PID, UID)
        val identityObjects = identityEntries + JM
        val entries = listOf(AV, BV, CV, Live, WeChat, PID, UID)
    ...
    data object JM : Identifiable {
        ...
        override fun verifyAndTrimHead(idString: String) = super.verifyAndTrimHead(idString)
            ...
            ?: idString.filter { it in SIMPLIFIED }.takeIf { it.length == 6 }
                ?.fold("") { acc, c -> acc + SIMPLIFIED.indexOf(c) }
            ?: idString.filter { it in TRADITIONAL }.takeIf { it.length == 6 }
                ?.fold("") { acc, c -> acc + TRADITIONAL.indexOf(c) }
    ...
    data object Unknown : Type {
        override val domain: String get() = throw IllegalStateException("Can not get domain for a unknown type.")
    }
}